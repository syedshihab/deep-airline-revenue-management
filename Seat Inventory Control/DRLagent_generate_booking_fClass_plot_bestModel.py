...
plt.plot(t1, nBookingsFclass0_eps, label='Fare class H', color='y')
plt.axhline(y=np.mean(np.array(nBookingsFclass0_eps)), color='b', linestyle='--', lw=1)
plt.plot(t1, nBookingsFclass1_eps, label='Fare class M', color='c') # forestgreen; limegreen; green; lime
plt.axhline(y=np.mean(np.array(nBookingsFclass1_eps)), color='b', linestyle='--', lw=1)
plt.plot(t1, beforeBumping_nBookingsFclass2_eps, label='Fare class L', color='lightcoral')
...
#plt.show()
plt.savefig(outputDir + '/testPlot_bestModel_bookingPlot.pdf')
plt.close()
