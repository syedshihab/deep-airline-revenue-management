...
# TODO: Before each run, change testData, change the pkl object's name if necessary before saving testLogger 
...
##                    AUcapacity = 0
                    # deteriming demand forecasts of each fare class for the remaining time of the booking period
                    ...
                    for fClassInd in range(nFareClasses):
                        ...
                        nArv = np.sum(lambdaValues[fClassInd,time:]) 
                        demandForecast_fClass[fClassInd] = b_fClass_t - nBookings_fClass[fClassInd] # total changeInB_fClass # # alternative equation: demandForecast_fClass[fClassInd] = changeInB


##                        if demandForecast_fClass[fClassInd]<0:
##                            # print('demandForecast_fClass[fClassInd]<0', 'time =', time, 'fClass =',fClassInd,'demandForecast_fClass[fClassInd] =',demandForecast_fClass[fClassInd])
##                            AUcapacity = AUcapacity + (-demandForecast_fClass[fClassInd])
##                            demandForecast_fClass[fClassInd] = 0
                        
                                        
                    paxCount = np.zeros(3, dtype=int)
                    
                    AUcapacity = int(AUcapacity + nSeatsAvailable)
                    ...
                    if (done):            
                        ...
                        if(sum(nBookings_fClass) > capacity): # Oversold/overbooked flight; need to bump some passengers
                            ...
                            if(nPaxToBump <= nBookings_fClass[nFareClasses-1]): # if(nPaxToBump <= nBookings_fClass[3]):
                                ...
                                reward -= min(overbooking_cost_multiplier*fClassPrices[nFareClasses-1],675/100)*nPaxToBump # DBC - Denied Boarding Compensation
                            elif((nPaxToBump-nBookings_fClass[nFareClasses-1]) <= nBookings_fClass[nFareClasses-2]):
                                # first bump all pax of lowest fare class
                                reward -= min(overbooking_cost_multiplier*fClassPrices[nFareClasses-1],675/100)*nBookings_fClass[nFareClasses-1]
                                ...
                                reward -= min(overbooking_cost_multiplier*fClassPrices[nFareClasses-2],675/100)*nPaxToBump
                                ...
                                reward -= min(overbooking_cost_multiplier*fClassPrices[nFareClasses-2],675/100)*nBookings_fClass[nFareClasses-2]
                                ...
                                reward -= min(overbooking_cost_multiplier*fClassPrices[nFareClasses-3],675/100)*nPaxToBump
                                # nPaxToBump = 0
                                nBookings_fClass[nFareClasses-3] -= nPaxToBump
            
            
                    # print('testLogger.observations[currentEpisodeIndex] =',testLogger.observations[currentEpisodeIndex],'\n')
                    # observation = (time+1, tuple(map(tuple, nBookings_fClass)), tuple(map(tuple, nSeatsSoldByPrices)), tuple(map(tuple, nCancellationsByPrices)))
                    # observation = (time+1, nBookings_fClass, nSeatsSoldByPrices, nCancellationsByPrices)
            ...
            ##        return observation, reward, done, dict()
                    ...
                    if nSeatsAvailable<0:
                        print('nSeatsAvailable is negative; flight is currently overbooked')
                        print('currentEpisodeIndex =',currentEpisodeIndex)
                        print('time =',time)
                        print('seatsBooked =',sum(nBookings_fClass))
            ...
            ##testLogger.rewardPercentage = []
            ##testLogger.loadFactor = []
