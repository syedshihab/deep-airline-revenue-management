...
with open('emsrbAgent_testLogger_cnc1_fcArrivals1_2fp22pp_testedWith_true_mktSiml3.pkl','rb') as fp2:  # Python 3: open(..., 'rb')
...
LF = np.array(test_logger.loadFactor)*100 # [:500]

RP_EMSRb = np.array(testLogger_EMSRb.rewardPercentage)*100
LF_EMSRb = np.array(testLogger_EMSRb.loadFactor)*100

plt.hist(RP,color='c')
plt.xlabel('Optimal revenue (%)')#,**tnrfont)
plt.ylabel('Count')#,**tnrfont)
plt.savefig('hist_RP_DRL.png')
plt.close()

plt.hist(RP_EMSRb,color='tab:orange')
plt.xlabel('Optimal revenue (%)')#,**tnrfont)
plt.ylabel('Count')#,**tnrfont)
plt.savefig('hist_RP_EMSRb.png')
plt.close()
##
##
...
##
### Plotting RP running mean
##plt.plot(t2, running_mean(RP, rmInterval), color='c', label='DRL agent')
##plt.axhline(y=np.mean(RP), color='b', linestyle='--', lw=1, label='DRL agent overall average')
##
##plt.plot(t2, running_mean(RP_EMSRb, rmInterval), color='tab:orange', label='EMSRb agent')
##plt.axhline(y=np.mean(RP_EMSRb), color='r', linestyle='--', lw=1, label='EMSRb agent overall average')
##
##plt.ylabel('Optimal revenue (%)')#,**tnrfont)
##plt.xlabel('Episode')#,**tnrfont)
##axes = plt.gca()
...
##plt.legend()
### plt.show()
##plt.savefig(outputDir + '/testPlot_RP_trueMktSiml_EMSRb_DRL.pdf')
##plt.savefig(outputDir + '/testPlot_RP_trueMktSiml_EMSRb_DRL.png')
##plt.close()
### plt.show()
##
##
...
##
### Plotting LF running mean
##plt.plot(t2, running_mean(LF, rmInterval), color='c', label='DRL agent')
##plt.axhline(y=np.mean(LF), color='b', linestyle='--', lw=1, label='DRL agent overall average')
##
##plt.plot(t2, running_mean(LF_EMSRb, rmInterval), color='tab:orange', label='EMSRb agent')
##plt.axhline(y=np.mean(LF_EMSRb), color='r', linestyle='--', lw=1, label='EMSRb agent overall average')
##plt.ylabel('Load factor (%)') #,**tnrfont)
##plt.xlabel('Episode') #,**tnrfont)
##axes = plt.gca()
###axes.set_xlim([1,nTestEpisodes])
...
##plt.legend()
##plt.savefig(outputDir + '/testPlot_LF_trueMktSiml_EMSRb_DRL.pdf')
##plt.savefig(outputDir + '/testPlot_LF_trueMktSiml_EMSRb_DRL.png')
##plt.close()
### plt.show()
##
