% true market simulator
% generate historical data for estimating market parameters
% generate test data
...
% nEpisodes = 15000; % 85000; % 50000; % 5000; % 500000; % 400000; % 200000; % 1000; % 200000; % 10000; % 200000; % 25000; % number of episodes used for training the DRL agent
...
for meanNarrivalsSetting=1:1
    for cancelProbsSetting=1:1        
        ...
        % fileName = 'historicalData_cnc' + string(cancelProbsSetting) + '_fcArrivals' + string(meanNarrivalsSetting) + '_2fp22pp_true_mktSiml'; 
...
%         % previous line and uncomment this line; also, change nEpisodes to the desired number of test episodes (e.g. 1000)
        ...
        nCancellations_by_wtpGroup_arvTime_eps = zeros(nWTPgroups,bookingHorizon,nEpisodes);
        for eps=1:nEpisodes
                ...
                if arrivalTime<182 % assumption: passengers do not cancel in the time step in which they made a booking
                    for timeInd=(arrivalTime+1):182
                        ...
                        if willCancel==1
                            ...
                            nCancellations_by_wtpGroup_arvTime_eps = nCancellations_by_wtpGroup_arvTime_eps(1,arrivalTime,eps) + 1;
                            break;
                        end
                    end
                end
                ...
                if arrivalTime<182 % assumption: passengers do not cancel in the time step in which they made a booking
                    for timeInd=(arrivalTime+1):182
                        ...
                        if willCancel==1
                            ...
                            nCancellations_by_wtpGroup_arvTime_eps = nCancellations_by_wtpGroup_arvTime_eps(2,arrivalTime,eps) + 1;
...
%         nCommittedPassengers_wtpGroup_time_eps = nArrivals - nCancellations_by_wtpGroup_time_eps;
        
        trDataPy = cellfun(@(c) c-1, trData, 'UniformOutput', false);  
        nArrivals_eachEps = cellfun(@length, trDataPy);
        
        
        % Computing max reward using approach 1
...
%         % Computing max reward using approach 2
%         maxReward_eps = func_compute_maxRevenue_eps(nArrivals, nCancellations_by_wtpGroup_time_eps);
