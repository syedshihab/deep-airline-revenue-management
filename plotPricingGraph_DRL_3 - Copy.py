...
filepath = "output_OB2cnc1fd1_2fp22pp_twg1wg4c1c2_nHL3nHN256tmu1lr1pol123_bestAgent"

test_logger = infoLogger()

with open(filepath + '/testLogger_bestModel_trueMktSiml_lrNeg4_tmu10k.pkl','rb') as filepointer1:  # Python 3: open(..., 'rb')
    # print(f)
...
testData_filename = "testData_cnc1_fcArrivals1_2fp22pp_true_mktSiml.mat"
...
fp2closed_timeSteps = np.where(np.array(fares_fc2) == np.inf)
...
plt.savefig(outputDir + '/plot_pricing_bestEps_DRL.pdf')
plt.savefig(outputDir + '/plot_pricing_bestEps_DRL.png')
plt.close()
# plt.show()
...
MATLABepisodeIndex = epsMaxRPind + 1 
...
plt.subplot(211)
plt.plot(t1, np.array(fares_fc1)*100, label='High fare product')
plt.plot(t1, np.array(fares_fc2)*100, label='Low fare product', color='tab:orange') # , t1, fares_fc3)
plt.plot(np.arange(32,42), [150]*10, color='tab:orange', linestyle='dotted') # , t1, fares_fc3)
plt.plot(np.arange(75,79), [150]*4, color='tab:orange', linestyle='dotted') # , t1, fares_fc3)
plt.plot(np.arange(82,84), [150]*2, color='tab:orange', linestyle='dotted') # , t1, fares_fc3)
plt.plot(np.arange(83,85), [150,200], color='tab:orange') # , t1, fares_fc3)
plt.plot(np.arange(147,161), [200]*14, color='tab:orange', linestyle='dotted') # , t1, fares_fc3)
# plt.title('dynamicPricingGraph_bestPerformance')
##plt.legend(loc='best')
...
plt.savefig(outputDir + '/plots_pricing_paxArrival_v2.pdf')
plt.savefig(outputDir + '/plots_pricing_paxArrival_v2.png')
####plt.show()
