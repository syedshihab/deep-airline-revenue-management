...
nTestEps = 2000 # 200 # 2000 # 5000 # 150
...
f = open("EMSRbAgent_numExpResults_" + repr(expInd) + ".txt", "w")
...
#################################################################################################################################################################################
            ...
            updateInterval_mktParameters = 1 # best value found from hyperparameter tuning experiments # 50
##            update_nMostRecentEps = 500
            update_nMostRecentObs = 1000 # # best value found from hyperparameter tuning experiments # 500 # TODO: vary
...
###################################################################################################################################################################
            ...
            mat = scipy.io.loadmat('historicalData_cnc1_fcArrivals1_2fp22pp_true_mktSiml', squeeze_me=True)
            data_nArrvials_wtpGrp_t_eps = mat['nArrivals']
            data_nArrvials_wtpGrp_t_eps2 = data_nArrvials_wtpGrp_t_eps[:,:,-update_nMostRecentObs:] # last update_nMostRecentObs episodes
            data_list_nArrvials_wtpGrp_t_eps = data_nArrvials_wtpGrp_t_eps2.tolist() # converting nd-array to list to be able to do appending and popping which changes the length of the list

            mat = scipy.io.loadmat('data_cncRate_wtpGrp_t', squeeze_me=True)
            data_cncRate_wtpGrp_t_eps = mat['data_cncRate_wtpGrp_t']
            data_cncRate_wtpGrp_t_eps2 = data_cncRate_wtpGrp_t_eps[:,-update_nMostRecentObs:] # last update_nMostRecentObs episodes
            data_list_cncRate_wtpGrp_t_eps = data_cncRate_wtpGrp_t_eps2.tolist() # converting array to list to be able to do appending and popping which changes the length of the list
...
###################################################################################################################################################################
                if ((currentEpisodeIndex+1)%updateInterval_mktParameters==0):
                    ...
                    # estimate parameters based on the most recent n flight episodes, where n = update_nMostRecentEps
##                    if currentEpisodeIndex<=update_nMostRecentEps:
                    for wtpInd in [0,2]: # range(0,4):
                        for t in range(0,182):
                            lambdaValues[wtpInd,t] = sum(data_list_nArrvials_wtpGrp_t_eps[wtpInd][t])/update_nMostRecentObs # update_nMostRecentObs = len(data_list_nArrvials_wtpGrp_t_eps[wtpInd][t])

                    for wtpInd in [1,3]: # range(0,4):
                        for t in range(0,182):
                            lambdaValues[wtpInd,t] = sum(data_list_nArrvials_wtpGrp_t_eps[wtpInd][t])/update_nMostRecentObs - lambdaValues[wtpInd-1,t]

                    lambdaValues[lambdaValues < 0] = 0 # ensuring that all lambdaValues are greater than or equal to zero

                    wtpInd = 0
                    for t in range(0,182):
                        cncProb_fClass_t[0][t] = sum(data_list_cncRate_wtpGrp_t_eps[t])/update_nMostRecentObs # average cancellation rate # running_nObs_PaxCnc_wtpGrp_t[0])
...
###################################################################################################################################################################
                    # updating data_list_nArrvials_wtpGrp_t_eps
                    if nBookings_atTminus1_Hfp>0:
                        data_list_cncRate_wtpGrp_t_eps[time].append(np.sum(nCncAtT_wtpGrp)/nBookings_atTminus1_Hfp) # insert latest observation                        
                        data_list_cncRate_wtpGrp_t_eps[time].pop(0) # remove oldest observation
###################################################################################################################################################################
...
###################################################################################################################################################################
                    ...
                    # high fp
                    fareInd = np.where(fareLevels == prices_fp[0])[0][0]
                    data_list_nArrvials_wtpGrp_t_eps[fareInd][time].append(count_nArv_at_t_fp[0]) # insert latest observation
                    data_list_nArrvials_wtpGrp_t_eps[fareInd][time].pop(0) # remove oldest observation; length of data_list_nArrvials_wtpGrp_t_eps remains fixed = update_nRecentObs
                    # low fp
                    if prices_fp[1]<10:
                        fareInd = np.where(fareLevels == prices_fp[1])[0][0]
                        data_list_nArrvials_wtpGrp_t_eps[fareInd][time].append(count_nArv_at_t_fp[1]) # insert latest observation
                        data_list_nArrvials_wtpGrp_t_eps[fareInd][time].pop(0) # remove oldest observation
###################################################################################################################################################################
...
##            print('avgLoadFactor = ' + repr(np.mean(LF)))
            ...
            pklFilename = "emsrbAgent_testLogger_cnc" + repr(cancelProbsSetting) + "_fcArrivals" + repr(meanNarrivalsSetting) + '_2fp22pp_testedWith_true_mktSiml' + repr(expInd) + ".pkl"            
...
##            with open("EMSRbAgent_mktEstimatesLatest" + repr(expInd) + ".pkl", 'wb') as pklFilePointer2:
##                pickle.dump([lambdaValues, cncProb_fClass_t, prevSum_nArrvials_wtpGrp_t + runningSum_nArv_wtpGrp_t, prevSum_cncRate_wtpGrp_t + runningSum_cncRate_wtpGrp_t, nEpisodes, running_nObs_PaxArv_wtpGrp_t, currentEpisodeIndex], pklFilePointer2)
            ...
            with open("EMSRbAgent_mktEstimatesLatest" + repr(expInd) + ".pkl", 'wb') as pklFilePointer2:
                pickle.dump([lambdaValues, cncProb_fClass_t, data_list_nArrvials_wtpGrp_t_eps, data_list_cncRate_wtpGrp_t_eps], pklFilePointer2)
