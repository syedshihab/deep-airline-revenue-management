...
data_fileName = 'historicalData_cnc' + string(1) + '_fcArrivals' + string(1) + '_2fp22pp_true_mktSiml'; 
load(data_fileName)

nEpisodes = 10000; % number of flight episodes in historical data; specify an even number

sum_nArrvials_wtpGrp_t = zeros(nWTPgroups,182);
% assuming a = [600, 200] in the first nEpisodes/2 episodes
lambdaValues_est = zeros(nWTPgroups,182);
for wtpGrpInd=[1,3]
    for t=1:182
        sum_nArrvials_wtpGrp_t(wtpGrpInd,t) = sum(nArrivals(wtpGrpInd,t,1:(nEpisodes/2)));
        lambdaValues_est(wtpGrpInd,t) = sum_nArrvials_wtpGrp_t(wtpGrpInd,t)/(nEpisodes/2);        
    end
end

% assuming a = [400, 100] in the last nEpisodes/2 episodes
for wtpGrpInd=[2,4] % lower wtpGrps of both fare products
    for t=1:182
        sum_nArrvials_wtpGrp_t(wtpGrpInd,t) = sum(nArrivals(wtpGrpInd,t,nEpisodes/2:nEpisodes)) + sum(nArrivals(wtpGrpInd-1,t,nEpisodes/2:nEpisodes));
        lambdaValues_est(wtpGrpInd,t) = sum_nArrvials_wtpGrp_t(wtpGrpInd,t)/(nEpisodes/2) - lambdaValues_est(wtpGrpInd-1,t);
    end
end

lambdaValues_est(lambdaValues_est<0)=0;


cncProb_wtpGroups_est = zeros(nWTPgroups,182);
nArrivals_cumulative = cumsum(nArrivals,2);
sum_cncRate_wtpGrp_t = zeros(nWTPgroups,182);
data_cncRate_wtpGrp_t = zeros(1,182,7500);

wtpGrpInd=1; 
for t=2:182
    data_cncRate_wtpGrp_t(wtpGrpInd,t,:) = nCancellations_by_wtpGroup_time_eps(wtpGrpInd,t,1:(nEpisodes/2))./(nArrivals_cumulative(wtpGrpInd,(t-1),1:(nEpisodes/2)));
    sum_cncRate_wtpGrp_t(wtpGrpInd,t) = sum(nCancellations_by_wtpGroup_time_eps(wtpGrpInd,t,1:(nEpisodes/2))./(nArrivals_cumulative(wtpGrpInd,(t-1),1:(nEpisodes/2))));     
    cncProb_wtpGroups_est(wtpGrpInd,t) = sum_cncRate_wtpGrp_t(wtpGrpInd,t)/(nEpisodes/2);
end

data_cncRate_wtpGrp_t(isnan(data_cncRate_wtpGrp_t))=0;
cncProb_wtpGroups_est(2,:) = cncProb_wtpGroups_est(1,:);

% wtpGrpInd=2; 
% for t=2:182        
%     cncProb_wtpGroups_est(wtpGrpInd,t) = sum(nCancellations_by_wtpGroup_time_eps(wtpGrpInd,t,nEpisodes/2:nEpisodes)./(nArrivals_cumulative(wtpGrpInd,(t-1),nEpisodes/2:nEpisodes)))/(nEpisodes/2);
% end

sum_cncRate_wtpGrp_t(isnan(sum_cncRate_wtpGrp_t))=0;
cncProb_wtpGroups_est(isnan(cncProb_wtpGroups_est))=0;

save('mktParameterEstimates.mat','lambdaValues_est','cncProb_wtpGroups_est','sum_cncRate_wtpGrp_t','sum_nArrvials_wtpGrp_t','nEpisodes','-v7');
...
% 
% fProductPricePoints = {[6, 5, 4], [2, 1.5, 1]}; % fare product prices will be set by agent
% % fProductBookingWindow = [7, 7, 7, 21, 42, 119]; 
% 
% nfProductPricePoints = cellfun(@length,fProductPricePoints); % = [3, 3]
% nWTPgroups = sum(nfProductPricePoints); % = 6
% 
% 
% fProductIdentifier = [];
...
% end
% fProductIdentifierPy = fProductIdentifier - 1; % [0 0 0 1 1 1]
% 
% wtp = [];
% for i=1:nFareProducts
...
% fareLevels = wtp; % = [400, 300, 250, 200, 125, 100] = cell2flatArray(fProductPricePoints)
% 
...
% end
% 
% % fProductDistrMeans = [8, 8, 20, 15, 20, 70]; % mean number of arrivals of passengers in different WTP groups/categories; % totalDemand = 1.5*capacity = 150 if capacity=100 
% % 
% % 
% % fd = 1; % fd = {1,2,3} fd gives the fare class passenger distribution setting 
% % fProductMeanDemand = fProductDistrMeans(fd,:); % here fProduct = WTP mean demand
% 
% % wtpGroupCancelRate = fProductCancelRate;
% % fProductCancelRate = [0.1, 0.1, 0.1, 0, 0, 0]; % , 0, 0]; % booking/ticket cancellation rates are different for each fare class;
% % cncFee = [0, 0]; % fProductCancelRefund = [1 0.5 0]; % lowest fare class passengers do not have the option of cancelling their booking
% 
% 
% 
% 
% wtpGroupMeanArrivals_allExp = [8, 8, 20, 20, 40, 60;...
%                              10, 15, 20, 20, 45, 45;...
%                              10, 15, 25, 25, 40, 40]; % the mean number of passenger arrivals of the different WTPgroups in three different settings (experiments); 
% % Note that mean totalDemand =~ 1.5*capacity =~ 150 in all experiments
% 
% 
% wtpGroupCncRate_last2timeSteps_allExp = [0.05, 0;...
%                                        0.1, 0;...
...
%         cncProb_wtpGroups = [repelem(0.0001,1,180), wtpGroupCncRate_last2timeSteps_allExp(cancelProbsSetting,1), wtpGroupCncRate_last2timeSteps_allExp(cancelProbsSetting,1);...
%                              repelem(0.0001,1,180), wtpGroupCncRate_last2timeSteps_allExp(cancelProbsSetting,1), wtpGroupCncRate_last2timeSteps_allExp(cancelProbsSetting,1);...
%                              repelem(0.0001,1,180), wtpGroupCncRate_last2timeSteps_allExp(cancelProbsSetting,1), wtpGroupCncRate_last2timeSteps_allExp(cancelProbsSetting,1)];
%         
%         
%         fileName = 'trData_cnc' + string(cancelProbsSetting) + '_fcArrivals' + string(meanNarrivalsSetting) + '_practice'; 
% %         fileName = 'testData_cnc' + string(cancelProbsSetting) + '_fcArrivals' + string(meanNarrivalsSetting) + '_practice'; % to generate test data, comment out the 
...
%                 nArrivals(wtpGroup,time,:) = poissrnd(lambdaValues(wtpGroup,time),1,nEpisodes);
%             end
%         end
%         
...
%             end
%             
%             for ind3=1:length(rowInd_wtpGroup3) % looping through all passengers of fare class 2
%                 % determining the cancellation time of passenger ind3 of wtpGroup3; if the passenger does not cancel, cancellationTime=0
%                 arrivalTime = trData{eps}(rowInd_wtpGroup3(ind3),1);
%                 if arrivalTime<182 % assumption: passengers do not cancel in the time step in which they made a booking
%                     for timeInd=(arrivalTime+1):182
%                         willCancel = binornd(1,cncProb_wtpGroups(3,timeInd)); % number of trials=1; passenger cancellation decision depends on cancellation probability
%                         if willCancel==1
%                             trData{eps}(rowInd_wtpGroup3(ind3),3) = timeInd; % setting cancellation time in trData
%                             nCancellations_by_eps_wtpGroup(eps,3) = nCancellations_by_eps_wtpGroup(eps,3) + 1;
%                             break; % no need to check for occurence of cancellation in other time steps
%                         end
%                     end
%                 end
