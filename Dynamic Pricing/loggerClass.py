class infoLogger_emsrb: # TODO: Generate graphs showing how the agent varies price of the fare products with time in each episode
    ...
    def __init__(self):
        ...
        self.seats_atEndEachEps = [] # self.fclassSeatsAllocation or self.fclassSeatsOccupancy at the end of each episode
    ...
    def on_step_end(self, eps, obs, reward, prices, BLemsrb): # callbacks.on_step_end(episode_step, step_logs)

        # print('testLogger.observations[eps] =',self.observations[eps])
        self.observations[eps].append(obs)
        # print('testLogger.observations[eps] =',self.observations[eps])
        self.rewards[eps].append(reward)
        self.prices[eps].append(prices)
        self.BLemsrb[eps].append(BLemsrb)
        

    # compute RewardPercentage (as compared to optimalReward) and loadFactor of each episode 
    def on_episode_end(self, seats, nSeatsSoldByPrices, nCancellationsByPrices, episode_reward, max_reward, overbooking, capacity):
        self.seats_atEndEachEps.append(seats) # infoLoggerObj.seats = [seatsEps0, seatsEps1, ...]
        self.nSeatsSoldByPrices.append(nSeatsSoldByPrices) 
        self.nCancellationsByPrices.append(nCancellationsByPrices) 
        ...
        if(overbooking > 0): 
            ...
            self.loadFactor.append(sum(seats)/capacity)
