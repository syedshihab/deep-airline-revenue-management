...
class armEnv(Env): # armEnv is a subclass of the Env abstract class
    ...
    def __init__(self, filename, biased=1, computeRewardAtEnd=0, ob=2): 
        ...
        self.action_space = spaces.Discrete(self.priceMatrix.shape[0]) # action_space = {0,1,...,26}; action_space.n = 27
        ...
        self.nUnqFareClasses = mat['nUnqFareClasses']
        ...
        self.seats = np.zeros(self.nUnqFareClasses, dtype=int) # state variables: s1,s2,s3
        self.seatsSoldByPrices = np.zeros(self.nWTPgroups, dtype=int) # seatsSoldByPrices = [0 0 0 0 0 0]; seatsSoldByPrices[0] = number of seats sold at $400, seatsSoldByPrices[1] = number of seats sold at $300, ...
...
##                                               spaces.Box(low=np.zeros(self.nFareClasses), high=np.full(self.nFareClasses, 2*self.capacity), dtype=int)))
        ...
        # TODO: add discount factor of 0.99 to facilitate convergence
    ...
    def step(self, action):
        ...
        while (arvTime==self.time): # pax arrives within this time step
            ...
            if (self.wtp[wtpGroup]>=prices[fclass]): # pax buys the ticket
                ...
                # self.seatsSoldByPrices[wtpGroup] += 1
                ...
                if (cncTime > -1):
                    ...
                    self.seatsSoldByPrices[priceInd[0][0]] += 1 # At the end of the episode, sum(self.seats) = sum(self.seatsSoldByPrices) 
...
####                self.nextClass = -1; # TODO: no need for nextClass; remove it
        ...
        if (self.time==self.totalTime): # if self.time == 181
            self.done = True
##        else:
##            self.time = self.time + 1 # time advances one step in the episode
...
##                reward = np.dot(self.seats, self.fclassPrices) [b1..b6]*[wtp1...wtp6]
            ...
            if(sum(self.seats) > self.capacity): # Oversold/overbooked flight
                ...
                if(nPaxToBump <= self.nSeatsSoldByPrices[5]): # if(nPaxToBump <= self.seats[5]):
                    self.seats[2] -= nPaxToBump
                    self.nSeatsSoldByPrices[5] -= nPaxToBump
                    # Assuming the scheduled flight is a domestic one and bumped pax will experience 1 to 2 hour arrival delay
                    reward -= min(self.overbooking_cost_multiplier*self.fareLevels[5],675)*nPaxToBump # DBC - Denied Boarding Compensation
                elif((nPaxToBump-self.nSeatsSoldByPrices[5]) <= self.nSeatsSoldByPrices[4]):
                    ...
                    self.seats[2] -= self.nSeatsSoldByPrices[5]
                    self.nSeatsSoldByPrices[5] = 0
                    # then bump middle class pax until nPaxOnBoard = capacity
                    reward -= min(self.overbooking_cost_multiplier*self.fareLevels[4],675)*nPaxToBump
                    self.seats[2] -= nPaxToBump
                    self.nSeatsSoldByPrices[4] -= nPaxToBump
        ...
        if(self.biased):
            ...
            self.observation = (self.time, self.seats, 1)
        else:
            self.observation = (self.time, self.seats)
    ...
    def reset(self):
        ...
        self.seatsSoldByPrices = np.zeros(self.nWTPgroups, dtype=int)
...
class armProcessor(Processor):
    ...
    def process_observation(self, observation):
        ...
        if(self.biased):
            processed_observation = np.concatenate((observation[0], observation[1], observation[2]),axis=None) # processed_observation = ([time, seats[0], seats[1], seats[2], bias=1]) array
        else:
            processed_observation = np.concatenate((observation[0], observation[1]),axis=None) # processed_observation = ([time, seats[0], seats[1], seats[2]]) array
...
##            self.nAccepts += 1
    ...
    def on_episode_end(self, episode, logs={}):
        ...
        # pdb.set_trace()
        self.rewardPercentage.append(logs['episode_reward']/self.env.max_reward) # TODO: compare with EMSRb reward
