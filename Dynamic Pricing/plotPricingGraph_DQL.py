...
filepath = 'output_OB2cnc1fd1_555_re-ran55' # 'output_OB2cnc1fd1_55' # 'output_OB2cnc1fd1_53' # 'output_OB2cnc1fd1_11'
...
with open(filepath + '/testLogger.pkl','rb') as f:  # Python 3: open(..., 'rb')
    # print(f)
    test_logger.seats, test_logger.nSeatsSoldByPrices, test_logger.nCancellationsByPrices, test_logger.episode_reward, \
    test_logger.max_reward, test_logger.rewardPercentage, test_logger.overbooking, test_logger.loadFactor, test_logger.observations, \
    test_logger.rewards, test_logger.actions = pickle.load(f)
...
plt.savefig(outputDir + '/dynamicPricingGraph_bestPerformanceDQL.png')
plt.close()
# plt.show()
...
MATLABepisodeIndex = epsMaxRPind # TODO: adjust if necessary; add +1???
nTotalPax = trDataPy[epsMaxRPind].shape[0] # trDataPy[nTrEpisodes + epsMaxRPind].shape[0]
currentEpisode = trDataPy[epsMaxRPind] # trDataPy[nTrEpisodes + epsMaxRPind] # 10000 + 77
nArrivals_wtpGroups = {}
...
rects0 = plt.bar(t1, nArrivals_wtpGroups[0], bar_width, align='edge',
alpha=opacity, label='WTP = $600')

rects1 = plt.bar(t1 + bar_width, nArrivals_wtpGroups[1], bar_width, align='edge',
alpha=opacity, label='WTP = $400')

rects2 = plt.bar(t1 + 2*bar_width, nArrivals_wtpGroups[2], bar_width, align='edge',
alpha=opacity, label='WTP = $150')

rects3 = plt.bar(t1 + 3*bar_width, nArrivals_wtpGroups[3], bar_width, align='edge',
...
plt.legend()
plt.tight_layout()
plt.savefig(outputDir + '/fareEvolution_paxArrival_plots.png')
##plt.show()
